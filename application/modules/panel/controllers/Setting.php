  ...
  public function ajax_save_application() {
    ...
    if ($this->form_validation->run() === true) {
      ...
      if (isset($_FILES['app_logo']) && !empty($_FILES['app_logo']['name'])) {
        $upload1 = $cpUpload->run('app_logo', 'app', true, true, 'jpg|jpeg|png|gif|ico');
        if ($upload1->status === true) {
          $_POST['app_logo'] = $upload1->data->base_path;
        } else {
          echo json_encode(array('status' => false, 'data' => 'Logo : '.$upload1->data));
          die;
        };
      } else {
        $temp_logo = $this->SettingAppModel->getDetail('data', 'app_logo');
        if (empty($temp_logo->content)) {
          echo json_encode(array('status' => false, 'data' => 'The Logo field is required.'));
          die;
        };
      };
